...
// A stack is a container adapter (wrapper) which uses an underlying container
...
void report_stack(T& stk) {
    cout << "size: " << stk.size() << " top: " << stk.top()
         << endl;  // top element is the back of the underlying container
}
...
int main() {
    ...
    stack<int, vector<int>> stk1(
        v1);  // 2 args, a: the type of stack & b: type of underlying container
    report_stack(stk1);
