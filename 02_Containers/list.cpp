...
int main() {
    ...
    message("insert:");  // Goal is to insert 112 before the element with the
                         // value of 5
    list<int>::iterator it1 = l1.begin();  // Can also use auto
    while ((*++it1 != 5) && (it1 != l1.end()))
        continue;  // Skip (move along the list) until the value pointed to by
                   // the iterator is 5
    if (it1 != l1.end()) {
    ...
    l1.remove(8);  // This is a convenience function that will do the above task
                   // for us (ie search the list for all the elements with the
                   // given value & remove them)
    message("remove 8");
    printl(l1);
